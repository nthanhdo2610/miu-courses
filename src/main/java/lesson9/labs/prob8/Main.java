...
public class Main {
    ...
    private boolean findProduct(String prodName) {
        return orderItems.stream()
                .map(i -> Optional.ofNullable(i.getProduct()))
                .map(p -> p.isPresent() ? Optional.ofNullable(p.get().getProductName()) : Optional.empty())
                .anyMatch(n -> n.isPresent() && n.get().equals(prodName));
